...
// 요소에 이벤트 리스너 등록하기
// 이벤트: 마우스(click), 스크롤(scroll), 키보드(keydown(키가 눌렸을때)) 등 다양함
// addEventListener() 메소드:
// 해당 요소에 지정한 이벤트가 발생하는지 듣고 있다가
// 실제 이벤트가 발생하면 연결된 콜백 함수(Event Handler)를 실행
// boxEl.addEventListener('이벤트명', 함수);
boxEl.addEventListener('click', function (e) { // 관례적으로 event의 e를 따서함
  console.log('Click!!');

  // (중요) this와 event 객체
  console.log(this); // this: 이벤트 리스너에서의 this는 이벤트가 바인딩된 요소를 가리킴(예: <div>)
  console.log(e.target); // e.target: 실제로 클릭된 DOM 요소(예: <div> or 자식 요소) , 이벤트버블링이 나타나네.

  // classList 속성: HTML 요소와 class 속성에 대한 제어 명령이 포함
  // 추가하기
